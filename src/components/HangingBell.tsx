...
export const HangingBell: React.FC<HangingBellProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="w-2.5 h-1 sm:w-4 sm:h-1.5 bg-gradient-to-b from-[#8C5C0B] via-[#E5B839] to-[#6A4005] rounded-b-sm shadow-sm border-t border-[#FFE58F]/50" />
      
      {/* Brass Chain with Detailed Metallic Links */}
      <div className="flex flex-col items-center">
        <div className="w-[1.5px] sm:w-[2px] h-3 sm:h-8 md:h-12 bg-gradient-to-b from-[#8C5C0B] via-[#FFE58F] to-[#8C5C0B] shadow-sm relative">
          <div className="absolute inset-0 bg-[repeating-linear-gradient(to_bottom,#6A4005_0px,#6A4005_2px,#FFE58F_3px,#E5B839_5px)] opacity-90" />
        </div>
      </div>

      {/* Bell Structure (Swings on Click) */}
      <button
        type="button"
        onClick={handleRing}
        ...
        className={`group relative flex flex-col items-center cursor-pointer focus:outline-none focus-visible:ring-1 focus-visible:ring-[#FFE58F] rounded-b-full p-0.5 transition-transform duration-300 ${
          isRinging ? 'animate-bell-ring' : 'hover:scale-105 active:scale-95'
        }`}
      >
        {/* Divine Golden Glow on Hover / Ring */}
        <div 
          className={`absolute -inset-1.5 sm:-inset-3 rounded-full bg-gradient-to-r from-[#D4AF37]/30 via-[#FFA000]/30 to-[#FFE58F]/20 blur-md pointer-events-none transition-opacity duration-500 ${
        ...
        {isRinging && (
          <span className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <span className="w-8 h-8 sm:w-16 sm:h-16 rounded-full border border-[#FFE58F] animate-ping opacity-60" />
          </span>
        )}

        {/* Hanging Ring Top Loop */}
        <div className="w-2 h-2 sm:w-3.5 sm:h-3.5 rounded-full border border-[#E5B839] bg-[#5B101D] shadow-inner mb-[-1px] sm:mb-[-2px]" />

        {/* Authentic South Indian Temple Brass Bell SVG */}
        <svg 
          viewBox="0 0 100 120" 
          className="w-5 h-6 sm:w-8 sm:h-10 md:w-10 md:h-12 drop-shadow-[0_4px_10px_rgba(0,0,0,0.5)]"
          fill="none" 
